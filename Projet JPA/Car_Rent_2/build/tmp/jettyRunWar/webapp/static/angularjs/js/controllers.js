...
$http.get('http://localhost:8080/Car_Rent_2/car').
...
$scope.rent = function(car) {
	$http.delete('http://localhost:8080/Car_Rent_2/car/'+car.plateNumber).
	  success(function(data, status, headers, config) {
	  }).
	  error(function(data, status, headers, config) {
	  });
...
rentControllers.controller("ramenerController", function($scope,$http,$routeParams) {
	...
	$scope.getBack = function(plateNumber) {
		$http.put('http://localhost:8080/Car_Rent_2/car/'+plateNumber).
		  success(function(data, status, headers, config) {
		  }).
		  error(function(data, status, headers, config) {
		  });
