...
var bool =false;
 ...
 function pageselectCallback(page_index, jq){
   if(bool)
	 {
	     window.location.href='<%=basePath%>/myadmin/admin.action?pager.everyPage=<s:property value="pager.everyPage"/>&pager.currentPage='+page_index;
	 }
	...
	function add()
	{
	   window.location.href="<%=basePath%>/myadmin/admin!manager.action?op=add";
	}
	 
	function edit(baseid)
	{
	   window.location.href="<%=basePath%>/myadmin/admin!manager.action?op=edit&baseid="+baseid;
	}
	...
	function btDel(baseid)
	{
	  ...
	  {
		 window.location.href='<%=basePath%>/myadmin/admin!commit.action?op=delete&baseid='+baseid;
	  }
	}
</script>
...
<body>
	...
	<div class="rightinfo">
		...
		<table class="tablelist">
			...
			<tbody>
			<s:if test="admins.size()!=0">
			<s:iterator id="admin" value="admins" status="rowNum">
				<tr>
					<td><s:property value="#rowNum.index + 1"/></td>
					<td><s:property value="#admin.username"/></td>
					<td><s:property value="#admin.password"/></td>
					<td>
					   <a href="javascript:void(0)" class="tablelink" onclick="edit('<s:property value="#admin.baseid"/>')">修改</a>
		               <a href="javascript:void(0)" class="tablelink" onclick="btDel('<s:property value="#admin.baseid"/>')">删除</a>
					</td>
				</tr>
			</s:iterator>
			</s:if>	
