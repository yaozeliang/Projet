		...
		for(var j = 0; j < relatedlink.length; j++) {
			if(relatedlink[j] && !relatedid[j]) {
				if(relatedlink[j]['surl'] != '') {
					var ra = '<a href="../法国学生汇-来法和在法留学生新生代社区_files/'+relatedlink[j]['surl']+'" target="_blank" class="relatedlink">'+relatedlink[j]['sname']+'</a>';
						alink[alink_i] = ra;
						ra = '#alink '+alink_i+'#';
						alink_i++;
				} else {
...
function _checksec(type, idhash, showmsg, recall, modid) {
	...
	x.loading = '';
	$('checksec' + type + 'verify_' + idhash).innerHTML = '<img src="../法国学生汇-来法和在法留学生新生代社区_files/'+ IMGDIR + '/loading.gif" width="16" height="16" class="vm" />';
	x.get('misc.php?mod=sec' + type + '&action=check&inajax=1&modid=' + modid + '&idhash=' + idhash + '&secverify=' + (BROWSER.ie && document.charset == 'utf-8' ? encodeURIComponent(secverify) : secverify), function(s){
		...
		if(s.substr(0, 7) == 'succeed') {
			obj.innerHTML = '<img src="../法国学生汇-来法和在法留学生新生代社区_files/'+ IMGDIR + '/check_right.gif" width="16" height="16" class="vm" />';
			if(showmsg) {
				recall(1);
			}
		} else {
			obj.innerHTML = '<img src="../法国学生汇-来法和在法留学生新生代社区_files/'+ IMGDIR + '/check_error.gif" width="16" height="16" class="vm" />';
			if(showmsg) {
				if(type == 'code') {
...
function _setDoodle(fid, oid, url, tid, from) {
	...
	if($(oid) != null) {
		if(from == "editor") {
			...
			if(wysiwyg) {
				insertText('<img src="../法国学生汇-来法和在法留学生新生代社区_files/' + url + '" border="0" alt="" />', false);
			} else {
...
function _zoom(obj, zimg, nocover, pn, showexif) {
	...
	var showimage = function (zimg, w, h, imgw, imgh) {
		...
		$(menuid + '_img').innerHTML = '<img id="' + zoomid + '" src="../法国学生汇-来法和在法留学生新生代社区_files/' + zimg + '" width="' + w + '" height="' + h + '" w="' + imgw + '" h="' + imgh + '" />' + imgtitle;
		if($(menuid + '_imglink')) {
...
function _showCreditmenu() {
	if(!$('extcreditmenu_menu')) {
		...
		menu.innerHTML = '<div class="p_opt"><img src="../法国学生汇-来法和在法留学生新生代社区_files/'+ IMGDIR + '/loading.gif" width="16" height="16" class="vm" /> 请稍候...</div>';
		$('append_parent').appendChild(menu);
...
function _createPalette(colorid, id, func) {
	var iframe = "<iframe name=\"c"+colorid+"_frame\" src=../法国学生汇-来法和在法留学生新生代社区_files/"/" frameborder=\"0\" width=\"210\" height=\"148\" scrolling=\"no\"></iframe>";
	if (!$("c"+colorid+"_menu")) {
...
function _setShortcut() {
	$('shortcuttip').onclick = function() {
		...
		msg += '2、点击"' + '<a href="../法国学生汇-来法和在法留学生新生代社区_files/forum.php?mod=misc&action=shortcut&type=ico" class="xi2 xw1">';
