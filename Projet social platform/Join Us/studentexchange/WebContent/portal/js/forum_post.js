...
function uploadAttach(curId, statusid, prefix, sizelimit) {
	...
	$(prefix + 'cpdel_' + nextId).innerHTML = '<img src="../【新提醒】发起活动 - 同城约 - 法国学生汇_files/' + IMGDIR + '/loading.gif" alt="上传中..." />';
	UPLOADSTATUS = 1;
...
function updatetradeattach(aid, url, attachurl) {
	$('tradeaid').value = aid;
	$('tradeattach_image').innerHTML = '<img src="../【新提醒】发起活动 - 同城约 - 法国学生汇_files/' + attachurl + '/' + url + '" class="spimg" />';
	ATTACHORIMAGE = 1;
}

function updateactivityattach(aid, url, attachurl) {
	$('activityaid').value = aid;
	$('activityattach_image').innerHTML = '<img src="../【新提醒】发起活动 - 同城约 - 法国学生汇_files/' + attachurl + '/' + url + '" class="spimg" />';
	ATTACHORIMAGE = 1;
}
...
function updatesortattach(aid, url, attachurl, identifier) {
	...
	$('sortattach_image_' + identifier).innerHTML = '<img src="../【新提醒】发起活动 - 同城约 - 法国学生汇_files/' + attachurl + '/' + url + '" class="spimg" />';
	ATTACHORIMAGE = 1;
}
